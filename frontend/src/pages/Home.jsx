                ...
                <div className='fixed bottom-0 left-0 w-full h-[80%] bg-white shadow-lg p-4 rounded-t-2xl overflow-hidden'>
                    ...
                    <div className='mb-6'>
                        ...
                        <div className='border p-2 rounded h-56 overflow-y-auto'>
                            {mockBookings
                                .filter(slot => slot.selectedDate === selectedDate)
                                .map(slot => (
                                    <div
                                        key={slot.slotId}
                                        className={`p-2 cursor-pointer ${selectedTime === slot.selectedTime ? 'bg-orange-400 text-white' : 'bg-gray-100'
                                            } rounded mb-1`}
                                        onClick={() => setSelectedTime(slot.selectedTime)}>
                                        {slot.selectedTime} - {slot.isBooked ? "Booked" : "Available"}
                                    </div>
                                ))}
                        </div>
